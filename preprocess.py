...
def read_instances_from_file(inst_file, max_sent_len, keep_case):
    ...
    with open(inst_file) as f:
        for sent in f:
            ...
            if not keep_case:
                sent = sent.lower() #converting the text to a lower case

         
            words = sent.split() #Splitting the words in to sentences
            ...
            if word_inst:
                word_insts += [[Constants.BOS_WORD] + word_inst + [Constants.EOS_WORD]]  #Add the current sentense to the list by adding BOS and EOS
    ...
    return word_insts  #list of lis which each list contain all the words in a single sentance with 

def build_vocab_idx(word_insts, min_word_count):
    ''' Trim vocab by number of occurence '''


    full_vocab = set(w for sent in word_insts for w in sent) #Filla list with all the words in the data set
